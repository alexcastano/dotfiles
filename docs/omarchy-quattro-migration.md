...
| 🔍 | VOX-5 | Retirar `playerctl` cuando sobre | Consecuencia de VOX-3: `playerctl` está instalado **a mano** y no lo pide ninguna dependencia, así que es un huérfano con fecha de caducidad. **Revisar cuando llegue voxtype 1.0.0** (o si se mergea el PR #7192): si ya no hace falta, desinstalar. Mientras tanto, las **otras máquinas** necesitan el mismo `pacman -S playerctl` a mano — no está apuntado en ningún instalador del repo por decisión explícita de dejar REP-7 en paz. |
